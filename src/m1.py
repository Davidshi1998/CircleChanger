...
class CircleChanger(object):
    ...
    def swell_or_shrink_once(self, amount_to_swell_or_shrink):
        ...
        x = random.randrange(3, 16)
        self.circle.outline_thickness = x

        x_index = random.randrange(0, len(self.colors))
        x_color = self.colors[x_index]
        self.circle.fill_color = x_color
    ...
    def swallow(self, other_circle_changer):
        ...
        b = self.circle.center.get_distance_from(a)
        return CircleChanger(a.x, a.y, b, 'red', self.colors + other_circle_changer.colors)
